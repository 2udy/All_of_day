...
while (player_index > 14) or (player_index < 0):
    print('-----해당 위치에는 배를 둘 수 없습니다.-----')
    player_index = int(input('배를 위치시킬 시작점을 고르세요. : '))

# 1-3) 컴퓨터의 배 시작 위치를 랜덤으로 지정합니다.
computer_index = random.randint(0,13)
...
while True:
    # 2-1) 플레이어 공격
# Information Board _ need to edit dashes line 
    print("--------------------------------------------")
    ...
    if player_attack_index > 16 or player_attack_index < 1:
        print('해역의 범위를 벗어난 위치를 선택하셨습니다. 다시 선택해 주세요.')
        continue
    if player_attacked[player_attack_index-1] == True:
        print('이미 공격한 위치를 선택하셨습니다. 다시 선택해 주세요')
    # 2-3) 플레이어의 공격이 성공한 경우
    if player_attacked[player_attack_index-1] == False:
    ...
    # 2-6) 컴퓨터의 공격이 성공한 경우
            if computer_attacked[computer_attack_index-1] == False:
                if computer_sea[computer_attack_index-1] == 1:
                    print(f'<{round}라운드 결과!>')
